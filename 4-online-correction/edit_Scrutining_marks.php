			...
			<div class="col-md-12"><br>
				...
				include('connect.php');
					...
					if(isset($_GET['stu_rollno']))
				    {
						...
						$query = mysqli_query($mysqli,"SELECT * From scrutiniser where fac_email='$fac_email' AND stu_midtype='$stu_midtype' AND stu_subject='$stu_subject'");
				...
				<form action="" method="POST" >				
				    ...
				    <?php
                            ...
                            if(isset($_POST['edit_marks']))
                            {
	                            ...
	                            $run_update = mysqli_query($mysqli,"UPDATE scrutiniser SET sub_tot_marks='$sub_tot_marks', obj_tot_marks='$obj_tot_marks' WHERE fac_email='$fac_email' AND stu_midtype='$stu_midtype' AND stu_subject='$stu_subject'");
									
	                                if($run_update)
	                                {
	                                    echo "<script>alert('$stu_rollno Marks Has Been Updated Successfully!')</script>";
                                        echo "<script>window.open('edit_Scrutining_marks?stu_rollno=$stu_rollno&stu_midtype=$stu_midtype&stu_subject=$stu_subject','_self')</script>";
	                                }
									else
									{
